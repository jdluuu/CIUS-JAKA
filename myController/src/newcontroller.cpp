...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<6; i++){
        pub_joint[i] = nh.advertise<std_msgs::Float64>("/jakaUr/joint"+std::to_string(i+1)+"_position_controller/command", 10);
    }
    
    ros::Publisher pub_velo[6];
    for(int i=0; i<6; i++){
        pub_velo[i] = nh.advertise<std_msgs::Float64>("/jakaUr/joint"+std::to_string(i+1)+"_position_controller/velocity", 10);
    }

    moveit::planning_interface::MoveGroupInterface arm("jakaUr");
